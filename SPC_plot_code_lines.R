...
plot_SPC_lines <- function(df, 
  ...
  #recalculate limits after rule break 3 - solid line
  ...
  pct_recal[end_point3:breakPoint4,"ucl"] <- pct_recal[end_point3, "ucl"]
  pct_recal[end_point3:breakPoint4,"lcl"] <- pct_recal[end_point3, "lcl"]
  ...
  if(chart_typ == "C"){
    pct_ruleBreak4 <- qicharts2::qic(date, value, data = df[breakPoint4:end_point4,],
                                     chart = 'c', exclude = exclude5)
  }else if(chart_typ == "C'"){
    pct_ruleBreak4 <- qicharts2::qic(date, value, n = rep(1, (extension_length4+1)), data = df[breakPoint4:end_point4,],
                                     chart = 'up', exclude = exclude5)
  }else if(chart_typ == "P'"){
    pct_ruleBreak4 <- qicharts2::qic(date, nonBreach, n = total, data = df[breakPoint4:end_point4,], 
                                     chart = 'pp', multiply = 100, exclude = exclude5)
  }else if(chart_typ == "P"){
    pct_ruleBreak4 <- qicharts2::qic(date, nonBreach, n = total, data = df[breakPoint4:end_point4,], 
                                     chart = 'p', multiply = 100, exclude = exclude5)
  }
  
  pct_ruleBreak4 <- pct_ruleBreak4$data %>%
    select(x,ucl,lcl, cl)
  pct_recal <- pct_recal %>%
    left_join(pct_ruleBreak4, by = "x") %>%
    rename(ucl = ucl.x, lcl = lcl.x, cl = cl.x) %>%
    mutate(ucl = ifelse(is.na(ucl), ucl.y, ucl)) %>%
    mutate(lcl = ifelse(is.na(lcl), lcl.y, lcl)) %>%
    mutate(cl = ifelse(is.na(cl), cl.y, cl))%>%
  ...
  if(plot_chart == T){
    ...
    format_SPC(pct, r1_col = r1_col, r2_col = r2_col) +
      ...
      geom_vline(xintercept = as.Date("2020-09-22"), colour = "red") + 
      annotate("text", x= as.Date("2020-09-23") + 1.75, y = line_height, 
               label = "Pubs & Restaurants \nclose at 10pm", angle = 90, size = 3, colour = "red") +
      geom_vline(xintercept = as.Date("2020-10-08"), colour = "red") + 
      annotate("text", x= as.Date("2020-10-09") + 1.75, y = line_height, 
               label = "Central Belt \nRestrictions", angle = 90, size = 3, colour = "red") +
      geom_vline(xintercept = as.Date("2020-10-30"), colour = "red") + 
      annotate("text", x= as.Date("2020-10-30") + 1.75, y = line_height, 
               label = "Start of SUSC", angle = 90, size = 3, colour = "red") +
      geom_vline(xintercept = as.Date("2020-12-26"), colour = "red") + 
      annotate("text", x= as.Date("2020-12-26") + 1.75, y = line_height, 
               label = "Tier 4 and Travel Ban", angle = 90, size = 3, colour = "red") +
      geom_vline(xintercept = as.Date("2021-01-04"), colour = "red") + 
      annotate("text", x= as.Date("2021-01-04") + 1.75, y = line_height, 
               label = "Further Lockdown", angle = 90, size = 3, colour = "red") 
...
format_SPC <- function(cht, r1_col, r2_col, ymin, ymax) {
  ...
  cht + 
    ...
    geom_line(aes(x,cl, linetype = limitType9), size = 0.75) +
    geom_line(aes(x,ucl, linetype = limitType9), size = 0.5) +
    geom_line(aes(x,lcl, linetype = limitType9), size = 0.5) +
